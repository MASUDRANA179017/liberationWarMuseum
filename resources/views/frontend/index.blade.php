                                    ...
                                    <div class="content">
                                        <div class="award__content">
                                            <h5 class="mb-3 text-center text-white title-animation fs-22 fw-700 lh-1">
                                                ...
                                                <a href="{{ $detailsRoute }}"
                                                    class="text-white">{{ Str::limit($exhibition->exhibition_title, 20, '...') }}</a>
                                            </h5>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="mt-5 text-center">
                        <a href="{{ route('exhibition.page') }}" aria-label="all exhibitions" title="all exhibitions"
        ...
        (function() {
            ...
            function calc() {
                ...
                if (!product_id || !service_id) {
                    setResult({
                        error: 'Select both product and service.'
                    });
                    return;
                }
                if (!(sft > 0)) {
                ...
                $.get(LOOKUP_URL, {
                    ...
                    .fail(xhr => setResult({
                        error: (xhr.responseJSON && xhr.responseJSON.message) || 'Lookup failed.'
                    }));
            }

            $(function() {
                $('#btnCalc').on('click', calc);
                $('#estProduct,#estService').on('change', () => {
                    if ($('#estSft').val()) calc();
                });
                $('#estSft').on('input', () => {
                    clearTimeout(window.__estDeb);
                    window.__estDeb = setTimeout(() => {
                        if ($('#estProduct').val() && $('#estService').val()) calc();
                    }, 250);
                });
                $('#btnResetEst').on('click', () => {
                    $('#priceEstimatorForm')[0].reset();
