                 ...
                 <div class="card card-round">
                    <div class="card-header project-details-card-header p-3">
                        <div class="d-flex align-items-center pb-2">
                            <h4 class="project-details-card-header-title mb-0"><i class='bx bxs-user-check'></i>Manage Management</h4>
                    ...
                    @include('admin.powerhouse-team.management-body.create-modal')
                    @include('admin.powerhouse-team.management-body.edit-modal')
                    <div class="card-body">
...
<script>
    ...
    function submitCreateForm() {
        ...
        $.ajax({
            url: "{{ route('management.body.store') }}",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    function submitEditForm() {
        ...
        $.ajax({
            url: "{{ route('management.body.update', '') }}/" + memberId,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    //     $.ajax({
    //         url: "{{ route('management.body.edit', '') }}/" + memberId,
    //         type: 'GET',
    //         success: function(response) {
    ...
    $(document).on('click', '.edit-member', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const serialOptions = response.serialOptions;

                $('#edit_member_id').val(member.id);
                $('#edit_name').val(member.name);
                $('#edit_member_code').val(member.member_code);
                $('#edit_department').val(member.department);
                $('#edit_designation').val(member.designation);
                $('#edit_dob').val(member.dob);
                $('#edit_joining_date').val(member.joining_date);
                $('#edit_gender').val(member.gender);
                $('#edit_blood_group').val(member.blood_group);
                $('#edit_phone').val(member.phone);
                $('#edit_email').val(member.email);
                $('#edit_whatsapp').val(member.whatsapp);
                $('#edit_facebook').val(member.facebook);
                $('#edit_linkedin').val(member.linkedin);
                $('#edit_github').val(member.github);
                $('#edit_address').val(member.address);
                $('#edit_about').val(member.about);
                $('#edit_image').val('');

                if (member.image) {
                    $('#edit_image_preview').attr('src', "/storage/" + member.image);
                }

                // Serial Dropdown Populate
                const $serialDropdown = $('#edit_serial');
                $serialDropdown.empty();
                $serialDropdown.append('<option value="">Select Serial Number</option>');

                serialOptions.forEach(serial => {
                    const selected = serial == member.serial ? 'selected' : '';
                    $serialDropdown.append(`<option value="${serial}" ${selected}>${serial}</option>`);
                });
    ...
    $(document).on('click', '.delete-member', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('management.body.destroy', '') }}/" + memberId,
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
    ...
    $(document).on('change', '.status-toggle', function() {
        ...
        $.ajax({
            url: "{{ route('management.body.status', '') }}/" + memberId,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
